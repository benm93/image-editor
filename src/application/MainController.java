...
public class MainController {
	...
	void sliderChange(MouseEvent event) {
		...
		viewer.setEffect(colorAdjust);
		/*
		 * for (int readY = 0; readY < image.getHeight(); readY++) { for (int readX = 0;
		 * readX < image.getWidth(); readX++) { Color color =
		 * pixelReader.getColor(readX, readY);
		 * System.out.println("\nPixel color at coordinates (" + readX + "," + readY +
		 * ") " + color.toString()); System.out.println("R = " + color.getRed());
		 * System.out.println("G = " + color.getGreen()); System.out.println("B = " +
		 * color.getBlue()); System.out.println("Opacity = " + color.getOpacity());
